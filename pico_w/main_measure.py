...
from machine import Pin, UART, WDT # type: ignore
...
def uart_ir_e350(DBGCFG:dict, uart_ir, wdt):
...
while True:
    ...
    debug_wdtFeed(wdt=wdt, DBGCFG=DBGCFG)
    wlan_connect(wdt=wdt, DBGCFG=DBGCFG, wlan=wlan, led_onboard=led_onboard, meas=True) # try to connect to the WLAN. Hangs there if no connection can be made
