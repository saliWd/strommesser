...
def getStatic(driver, testNum):
  from functions import checkSiteTitleAndPrint, doLoginCorrect, printOkOrNot
  subTest = 1
  
  driver.get("https://strommesser.ch/verbrauch/login.php")
  time.sleep(2)
  sourceCode = driver.page_source
  writeFile(fileName='staticHtml/login.php.static', fileContent=sourceCode)    
  ...
  for range in ranges:
    ...
    writeFile(fileName='staticHtml/index.php.'+range+'.static', fileContent=sourceCode)    
  ...
  time.sleep(2)
  sourceCode = driver.page_source
  writeFile(fileName='staticHtml/settings.php.static', fileContent=sourceCode)    
